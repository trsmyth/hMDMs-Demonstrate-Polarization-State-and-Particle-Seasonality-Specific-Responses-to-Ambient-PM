...
# Analysis of CellTox Green Cyotoxicity Assay Results
...
```{r}
# Import data
Celltox <- read.csv("Cytotoxicity.csv")

colnames(Celltox)[1] <- 'Particle_month'

# Rename particle months to full month name
Celltox$Full_name <- paste0(Celltox$Sample.ID, "_", Celltox$Particle_month)
Celltox <- data.frame(Celltox, row.names = Celltox$Full_name)
Celltox <- Celltox %>% subset(select = -c(Full_name))

Celltox$Particle_month[Celltox$Particle_month == 'Sept'] <- 'September'
Celltox$Particle_month[Celltox$Particle_month == 'Dec'] <- 'December'

# Retain a raw version of the Celltox data for fold change analysis below
Celltox_Raw <- Celltox

# Pivot data to long format
Celltox <- Celltox %>% 
  pivot_longer(cols = colnames(Celltox[3:ncol(Celltox)]),
               names_to = 'Treatment', 
               values_to = 'MFI')

Celltox$Sample.ID <- paste0(Celltox$Sample.ID, '_', Celltox$Particle_month)

Celltox$Treatment <- factor(Celltox$Treatment,
                                 levels = c('M0_Veh', 'M2_Veh',
                                            'M0_M1', 'M2_M1'))

Celltox$Particle_month <- factor(Celltox$Particle_month,
                                      levels = c('December', 'March', 'June', 'September'))

Celltox <- Celltox %>% subset(Treatment == 'M0_Veh' |
                                          Treatment == 'M2_Veh' |
                                          Treatment == 'M0_M1' |
                                          Treatment == 'M2_M1')
```

### Calculate statistics and graph results for Supplemental Figure S1C

```{r}
# Isolate Vehicle exposed samples for Supplemental Figures S1A and S1C
Vehicle <- Celltox %>% 
...
Supplemental_Figure_S1C <- ggplot(data = Vehicle, 
                                  aes(x = Treatment, 
                                      y = MFI)) + 
...
Supplemental_Figure_S1C_Statistics <- ggplot(data = stats, 
                                             aes(x = group1,
                                                 y = pos_y)) +
...
tiff('Supplemental_Figure_S1C.tiff', height = 1000, width = 1500)

wrap_plots(Supplemental_Figure_S1C_Statistics +  
             theme_void(),
           Supplemental_Figure_S1C + 
...
### Calculate statistics and graph results for Supplemental Figure S1D
...
# Generate Supplemental Figure S1C
Supplemental_Figure_S1D <- ggplot(data = Vehicle, 
...
Supplemental_Figure_S1D_Statistics <- ggplot(data = Month_stats, 
                                             aes(x = group1,
                                                 y = pos_y)) +
...
tiff('Supplemental_Figure_S1D.tiff', height = 1000, width = 1500)

patchwork::wrap_plots(Supplemental_Figure_S1D_Statistics +  
                        theme_void(), 
                      Supplemental_Figure_S1D + 
...
### Calculate the fold change Celltox MFI of PM exposed samples versus vehicle exposed samples

```{r}
# Isolate individual PM treatments and matched vehicle controls
M0_Veh <- Celltox_Raw %>% select(all_of(c('M0_Veh', 'M0_PM')))
M2_Veh <- Celltox_Raw %>% select(all_of(c('M2_Veh', 'M2_PM')))
M0_M1 <- Celltox_Raw %>% select(all_of(c('M0_M1', 'M0_M1.PM')))
M2_M1 <- Celltox_Raw %>% select(all_of(c('M2_M1', 'M2_M1.PM')))
...
colnames(FC) <- names
rownames(FC) <- rownames(Celltox_Raw)
FC <- data.frame(Celltox_Raw[1:2], FC)
...
save(FC, 
     file = 'Fold_Change_Cytotoxicity.RData')
```

### Calculate statistics and graph results for Figure 1C
...
Figure_1C <- ggplot(data = FC, 
  ...
  ylab("Fold Change Vehicle Cytotoxicity \n (Percent Lysis MFI)") +
...
Figure_1C_Statistics <- ggplot(data = stats, 
                               aes(x = group1,
                                   y = pos_y)) +
...
tiff('Figure_1C.tiff', height = 1000, width = 1500)

wrap_plots(Figure_1C_Statistics +  
             theme_void(),
           Figure_1C + 
             theme(legend.position = "none"), 
           ncol = 1,
...
### Calculate statistics and graph results for Figure 1D
...
Figure_1D_Statistics <- ggplot(data = Month_stats, 
                               aes(x = Month,
                                   y = pos_y)) +
...
tiff('Figure_1D.tiff', height = 1000, width = 1500)

patchwork::wrap_plots(Figure_1D_Statistics +  
                        theme_void(), 
                      Figure_1D + 
                        theme(strip.text.x = element_text(size = 25)), 
                      ncol = 1,
